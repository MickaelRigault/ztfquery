...
def avoid_duplicate(string_array):
    ...
    for name_, nname_ in occurence.items():
        if nname_ > 1:
            string_array[np.in1d(string_array, name_)] = [
                name_ + f"{i+1}" for i in range(nname_)
            ]
...
def sph2cart(vec):
    ...
    return np.asarray(
        [v * np.cos(b) * np.cos(l), v * np.cos(b) * np.sin(l), v * np.sin(b)],
        dtype=object,
    )
...
def cart2sph(vec):
    ...
    return np.array(
        [v, (np.arctan2(y, x) / _DEG2RA + 180) % 360 - 180, np.arcsin(z / v) / _DEG2RA],
        dtype=object,
    )
...
def rot_xz(v, theta):
    ...
    """
    x = v[0] * np.cos(theta * _DEG2RA) - v[2] * np.sin(theta * _DEG2RA)
    y = v[1]
    z = v[2] * np.cos(theta * _DEG2RA) + v[0] * np.sin(theta * _DEG2RA)
    return np.asarray([x, y, z], dtype=object)
