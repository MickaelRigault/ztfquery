...
def get_scifile_of_filename(filename, qid=None, source="irsa"):
    ...
    if parsing["kind"] == "raw":
        if qid is None:
            return [
                science_path(qid=str(i), **prop, source=source) for i in range(1, 5)
            ]
...
def parse_rawfilename(filename):
    """ """
    _, filefracday, paddedfield, filtercode, ccd_, suffix_ = os.path.basename(
        filename
    ).split("_")
...
def _localsource_to_source_(localfilepath, source=None):
    ...
    if LOCALSOURCE not in localfilepath:
        raise ValueError(
            "Cannot parse the location of the given file. %s, %s expected to be part of the path"
    ...
    if np.any([k in localfilepath for k in ZTFIRSA_BASE]):
        ...
        warnings.warn(
            "Cannot recover the online url, only IRSA ZTF files are implemented."
        )
        return None, "None"
...
def science_path(
    ...
    if suffix is None:
        ...
        raise ValueError(
            f"Unkwown suffix {suffix} for 'sci' exposures",
            "\n known suffixes: \n",
            KNOWN_SCIENCE_SUFFIXES,
        )
...
def calibration_path(
    ...
    if caltype.lower() not in ["bias", "hifreqflat"] and check_suffix:
        raise ValueError(
            f"Unknown `caltype` {caltype}, caltype must be  'bias' or 'hifreqflat'"
        )
...
def build_filename_from_dataframe(dataframe, suffix="sciimg.fits"):
    ...
    paddedfield = (
        dataframe["fieldid"].astype("int").astype("str").str.pad(6, fillchar="0")
    )
    paddedccd = "c" + dataframe["ccdid"].astype("int").astype("str").str.pad(
        2, fillchar="0"
    )
    paddedqid = "q" + dataframe["qid"].astype("int").astype("str")
    filtername = dataframe["filterid"].apply(
        lambda x: "zg" if x == 1 else "zr" if x == 2 else "zi"
    )
    return (
        "ztf"
        + "_"
...
def filename_to_url(filename, suffix=None, source="irsa", kind=None, **kwargs):
    ...
    **kwargs goes to the filename_to_{kind}url functions.
    ...
    if kind == "raw":
        return filename_to_rawurl(
            filename,
            **{k: prop[k] for k in ["imgtypecode", "source", "verbose"] if k in prop},
        )
...
def filename_to_scienceurl(
    filename, suffix=None, source="irsa", verbose=False, check_suffix=False
):
    """ """
    (
        _,
...
def fileroot_to_raw_url(
    fileroot, paddedccdid, imgtypecode="o", source="", verbose=False
):
    """
    Parameters
    ----------
